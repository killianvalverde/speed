 ...
 * @brief      child_cpu_chrono class methods definition.
...
bool child_cpu_chrono::start() noexcept
{
    if (chrn_state_ == chrono_states::READY)
    {
        ...
        speed::system::time::get_child_cpu_time(&start_time_spec_);
        return true;
    }
...
bool child_cpu_chrono::stop() noexcept
{
    if (chrn_state_ == chrono_states::RUNNING)
    {
        ...
        speed::system::time::get_child_cpu_time(&elapsed_time_spec_);
...
long double child_cpu_chrono::get_elapsed_time() const noexcept
{
    ...
    {
        speed::system::time::get_child_cpu_time(&cur_time_spec);
...
speed::system::time::time_specification child_cpu_chrono::get_elapsed_raw_time() const noexcept
{
    speed::system::time::time_specification elapsed_tme;

    if (chrn_state_ == chrono_states::RUNNING)
    {
        speed::system::time::get_child_cpu_time(&elapsed_tme);
