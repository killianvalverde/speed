...
#ifndef SPEED_ARGPARSE_DETAIL_BASIC_ARG_CONSTRAINT_HPP
#define SPEED_ARGPARSE_DETAIL_BASIC_ARG_CONSTRAINT_HPP
...
public:
    ...
    basic_arg_constraint(arg_parser_type* arg_parsr, const StringTs_&... kys)
            : arg_parsr_(arg_parsr)
    {
        base_arg_type* bse_arg;
        
        bse_args_.reserve(sizeof...(kys));
    ...
    /**
     * @brief       Allows knowing whether the constraint constains any of the specified arguments.
     * @param       bse_args : The objects to check whether they are contained.
     * @return      If function was successful true is returned, otherwise false is returned.
     */
    [[nodiscard]] bool contains_any_of(vector_type<base_arg_type*> bse_args) const noexcept
    {
        return std::ranges::any_of(bse_args, [&](base_arg_type* bse_arg) {
            return std::ranges::find(bse_args_, bse_arg) != bse_args_.end();
        });
    }
    ...
    ) const
    {
        ...
        {
            desc = "Arguments are mutually exclusive and at least one is required.";
        }
        else if (is_mutually_exclusiv)
        {
            desc = "Arguments are mutually exclusive.";
        }
        else if (is_one_or_more_requird)
        {
...
private:
    /**
     * @brief       Inserts a base argument into the list if it is not already present.
     * @param       bse_arg : Pointer to the base argument to insert.
     */
    void insert_arg(base_arg_type* bse_arg)
    {
        if (std::ranges::find(bse_args_, bse_arg) == bse_args_.end())
        {
