...
#ifndef SPEED_ARGPARSE_DETAIL_BASIC_KEY_VALUE_ARG_HPP
#define SPEED_ARGPARSE_DETAIL_BASIC_KEY_VALUE_ARG_HPP
...
class basic_key_value_arg
        : public basic_key_arg<AllocatorT>
        , public basic_value_arg<AllocatorT>
{
public:
     ...
     */
    template<typename... StringTs_>
    void set_values_names(StringTs_&&... vals_names)
    {
        vals_names_.clear();
        (vals_names_.emplace_back(std::forward<StringTs_>(vals_names)), ...);
        ...
        value_arg_type::update_minmax_values(vals_names_.size(), vals_names_.size());
        usage_str_needs_refresh_ = true;
    }
...
private:
    ...
    bool usage_str_needs_refresh_ = true;
};
