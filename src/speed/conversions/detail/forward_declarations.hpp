 ...
 * @brief Internal forward declarations for the conversions module.
...
bool try_convert(const SourceT&, TargetT&, std::error_code* = nullptr) noexcept;

template<typename TargetT, typename SourceT>
TargetT convert(const SourceT&);
...
TargetT convert_or(const SourceT& arg, InvocableT&& callabl);
