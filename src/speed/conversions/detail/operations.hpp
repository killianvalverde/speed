...
/**
 * @file operations.hpp
 * @brief Internal helper operations for the conversions module.
...
/** @cond */
namespace speed::conversions::detail {

// C-string --> unsigned_integral
template<std::unsigned_integral TargetT>
bool try_convert(const char* arg, TargetT& res, std::error_code* err_code) noexcept
{
    const char* str = arg;
    const char* end;
    int base = 10;
...
) noexcept
{
    ...
    if (!try_convert(arg, tmp, err_code))
    {
        return false;
    }

    return try_convert(tmp.c_str(), res, err_code);
}
...
) noexcept
{
    ...
    if (!try_convert(arg, rep, err_code))
    {
        return false;
    }
...
) noexcept
{
    ...
    if (!system::codecs::convert_utf8_to_wstring(arg, res, err_code))
    {
        return false;
    }
...
) noexcept
{
    ...
    if (!system::codecs::convert_wstring_to_utf8(arg, res, err_code))
    {
        return false;
    }
...
) noexcept
{
    ...
    if (!try_convert(arg, static_cast<std::filesystem::path&>(res), err_code))
    {
        return false;
    }
...
) noexcept
{
    if constexpr (std::same_as<CharTraitsT, std::char_traits<CharT>>)
    {
        ...
        {
            return try_convert(arg.data(), res, err_code);
        }
    }
...
{
    return try_convert(arg.c_str(), res, err_code);
}
